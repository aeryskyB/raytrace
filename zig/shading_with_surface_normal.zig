...
fn hit_sphere(sphr_c: vec3, sphr_r: f32, r: ray) f32 {
    ...
    return (-b - std.math.sqrt(discr)) / (2 * a);
}

fn ray_color(r: ray, sphr_c: vec3, sphr_r: f32) color3 {
    const t: f32 = hit_sphere(sphr_c, sphr_r, r);
    if (t > 0) {
        var norm: vec3 = vec3.add_v(ray.at(r.orig, r.dir, t), vec3.scale(sphr_c, -1));
        norm = vec3.scale(norm, 1 / vec3.length(norm));
        const f: f32 = 0.5 * 255;
        var c = color3.init_rgb(f * (norm.coord[0] + 1), f * (norm.coord[1] + 1), f * (norm.coord[2] + 1));
        color3.clamp_(&c);
        return c;
    }

    const v: vec3 = vec3.scale(r.dir, 1 / vec3.length(r.dir));
    const a: f32 = 0.45 * (v.coord[1] + 1);
